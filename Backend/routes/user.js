...
router.post('/signup', (req, res) => {
    ...
    connection.query(query, [user.email], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
                query = "INSERT INTO user(user_firstName,user_email,user_password,user_status,userRole_idRol) VALUES (?,?,?,'true',?)"
                connection.query(query, [user.name, user.email, user.password, user.rol], (err, results) => {
                    if (!err) {
